...
"""
This test verifies that dir_data, num_solute_atoms and atom_types are being assembled correctly from an input geometry.
Test functions exist for each spectroscopy as well as geometry optimization, in principle the outputs with each of these parameters are identcal.
"""

def test_coords_from_input_ROA_VCD():
    cwd = os.getcwd()
    # Molecule's geometry.
    input_geometry = """ 
    ...
    with open(f"{cwd}/SoAPy/tests/test_002_coords_from_input/002ref-ROA-VCD.txt", "r") as file:
        for line in file:
...
def test_coords_from_input_OptRot():
    cwd = os.getcwd()
    # Molecule's geometry.
    input_geometry = """ 
        1   O    0.000000    0.000000    0.000000
    ...
    with open(f"{cwd}/SoAPy/tests/test_002_coords_from_input/002ref-OptRot.txt", "r") as file:
        for line in file:
            split_line = line.split("#")
        assert split_line[1] == str(dir_data)
        assert split_line[2] == str(num_solute_atoms)
...
def test_coords_from_input_OptRot():
    ...
    with open(f"{cwd}/SoAPy/tests/test_002_coords_from_input/002ref-optimization.txt", "r") as file:
        for line in file:
            split_line = line.split("#")
        assert split_line[1] == str(dir_data)
        assert split_line[2] == str(num_solute_atoms)
