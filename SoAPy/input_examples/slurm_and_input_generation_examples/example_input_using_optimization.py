...
obtained molecular dynamics trajectory with 100 snapshots. This script includes an
...
trajectory_location = f'{cwd}/water_example_trajectory.arc'
...
#hdf5_location = f'{cwd}/{molecule_name}.h5'
#atoms, frames = SoAPy.functions.get_atoms_frames_from_GROMACS(molecule_name, trajectory_location)
t2 = time.time()

# Generate coordinate data from input geometry.
dir_data, num_solute_atoms, all_atom_types, solute_atom_types, solvent_atom_types = SoAPy.get_geometry.coordinates_from_trajectory(atoms, frames, hdf5_location, dir_list, dir_parameters)
t3 = time.time()
...
SoAPy.generate_files.modify_input(molecule_name, gaussian_parameters, dir_list, dir_parameters, dir_data, num_solute_atoms, all_atom_types, solute_atom_types, solvent_atom_types)
t6 = time.time()
